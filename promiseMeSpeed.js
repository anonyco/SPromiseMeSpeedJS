...
(function(){
	...
	const promiseBufferLvl = 32; // length of the promise buffer for double stack buffering
	...
	function thenableSPromise(func){
		// the thenableSPromise is a faster nonstacksafe version of SPromise that is internally used by .thens
		...
		var retObj = {
			"then": function(tFunc, cFunc){
				...
				if (stage === undefined){
					return thenableSPromise(function(accept, reject){
						if (tFunc){
					...
					if (tFunc) return resolve( tFunc(retVal) );
				} else {
					if (cFunc) return reject( cFunc(retVal) );
				}
			},
